...
# Identify tree with lowest log_loss
lowest_logloss_xgb <-
  metrics_xgb_all |>
  filter(sample == "Validation", recalib == "None") |>
  group_by(scenario, repn) |>
  ...
  slice_head(n = 1) |>
  select(scenario, repn, ind, nb_iter, recalib) |>
  mutate(result_type = "lowest_logloss") |>
  ungroup()
  ...
  mutate(
    result_type = factor(
      ...
      levels = c(
        ...
        "lowest_brier", "lowest_ici", "lowest_kl", "lowest_logloss"),
      labels = c(
...
plot_bp_xgb <- function(scenario,
  ...
  for (recalib_method in c("none", "platt", "iso")) {
    ...
    ## Min LL on validation set
    metrics_valid_ll_star <-
      metrics_xgb_current_valid |> arrange(log_loss) |> dplyr::slice(1)
    nb_iter_ll <- metrics_valid_ll_star$nb_iter
    max_depth_ll_star <- metrics_valid_ll_star$max_depth
    i_max_depth_ll_star <- which(max_depth_val == max_depth_ll_star)

    metrics_min_ll <-
      metrics_xgb_current_test |>
      filter(nb_iter == !!nb_iter_ll, max_depth == max_depth_ll_star)
    ind_ll <- which(map_dbl(scores_hist_repn[[i_max_depth_ll_star]], "nb_iter") == nb_iter_ll)
    scores_hist_min_ll <- scores_hist_repn[[i_max_depth_ll_star]][[ind_ll]]
    plot_bp_interest(
      metrics_interest = metrics_min_ll,
      scores_hist_interest = scores_hist_min_ll,
      label = "LL*",
      recalib_method = recalib_method
    )
...
df_plot <-
  ...
  select(
    ...
    brier, ici, KL_20_true_probas, log_loss
  ) |>
...
models_interest_xgb <- models_of_interest_metrics |> 
  ...
  summarise(
    ...
    KL_20_true_probas = mean(KL_20_true_probas),
    ll_lower = quantile(log_loss, probs = 2.5/100, na.rm = TRUE),
    ll_upper = quantile(log_loss, probs = 97.5/100, na.rm = TRUE),
    ll_sd = sd(log_loss, na.rm = TRUE),
    ll = mean(log_loss, na.rm = TRUE),
...
table_models_interest_mean <- 
  ...
  filter(
    result_type %in% c("AUC*", "KL*", "LL*", "Smallest", "Largest")
  ) |> 
...
table_models_interest_mean |> 
  ...
  pivot_longer(cols = c(brier, ici, kl, ll)) |> 
