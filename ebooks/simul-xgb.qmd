...
table_models_interest_mean <- 
  ...
  filter(sample == "test") |> 
  select(
    scenario, dgp, no_noise, recalib, sample, result_type, 
    brier, ici, kl = KL_20_true_probas
  ) |> 
  filter(
...
initial_points <- table_models_interest_mean |> 
  filter(dgp == 1, recalib == "None")

points_after_c <- initial_points |> 
  select(scenario, dgp, no_noise, result_type, ici, kl) |> 
  left_join(
    table_models_interest_mean |> 
      filter(recalib %in% c("Platt", "Isotonic")) |> 
      select(scenario, dgp, no_noise, recalib, result_type, ici, kl) |> 
