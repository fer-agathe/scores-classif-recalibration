...
```{r load-metrics_xgb_all.rda, echo=FALSE,eval=FALSE}
...
```{r define-scores_hist_all, eval=TRUE}
scores_hist_all <-
  map(
    resul_xgb,
...
```{r load-scores_hist_all, echo=FALSE, eval=FALSE}
...
p_kl_vs_ici_recalib_dgp_1 <- 
  ...
  # facet_grid(dgp~no_noise, scales = "free") +
  ggh4x::facet_grid2(
    dgp~no_noise, axes = "all", independent = "none", scales = "free_y"
  ) +
  scale_shape_manual(
    NULL,
    values = c(
